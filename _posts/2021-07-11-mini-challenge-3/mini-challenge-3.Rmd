...
##### 2.4.1.1 Getting the meaningful data.
...
```{r}
q2_fire_m <- q2_meaningful %>%
  filter(str_detect(message,"fire|upper|dolphin|dancing|building|apartment|Madeg|dispatch|afd|floor|floors|fireman|firefighters|firefighter|evacuate|evacuated|evacuating|evacuation|trapped|injuries|scene|trapped|collapsed|blaze|escalated|resident"))
...
q2_rally_tidy%>%
  filter(str_detect(word,"collapsed|blaze|escalated")) %>%
  ggplot(aes(x = time)) +
  geom_histogram()+
  theme(panel.grid=element_blank(),axis.text.x= element_text(angle=60, hjust= 1))+
  labs(y =NULL)
