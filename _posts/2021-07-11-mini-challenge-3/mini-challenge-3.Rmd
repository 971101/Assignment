...
```{r}
p <- q2_rally_tidy%>%
  filter(sub_detect(word,"pokrally|pok|rally|abila |city|park|stand|sylvia|marek|audrey|mcConnell|newman|professor|lorenzo|di|stefano|lucio|jakab|viktor-e|viktor-e")) %>%
  count(time,word,sort = TRUE) %>%
  ggplot(aes(x = time)) +
  geom_dotplot_interactive(aes(data_id = type),
                           stackgroups = TRUE,
                           binwidth = 1,
                           method = "histodot")+
