...
packages = c('readr','tidytext','data.table','lubridate','ggplot2',
             ...
             'DT','hms','ggiraph','topicmodels','raster','sf','maptools','rgdal')
for(p in packages){
...
The graph of <font color = red>*ccdata*</font> shows that the events like "fire","traffic".
...
pokrally,Abila City Park,Stand Up Speak Up
Sylvia Marek, Dr. Audrey McConnell Newman, Professor Lorenzo Di Stefano, Lucio Jakab and Viktor-E
...
q3_gps_m <- subset(q3,select = c("type","date(yyyyMMddHHmmss)","author",
                               "message","latitude","longitude","time"))
q3_gps_cc <- subset(q3,select = c("type","date(yyyyMMddHHmmss)","author",
                               "message","location","time"))
gps_m <- na.omit(q3_gps_m)
gps_sf <- st_as_sf(gps_m,
                   coords = c("longitude","latitude"),
                   crs = 4326)

gps_cc <- na.omit(q3_gps_cc)
```



```{r}
q3_author_lda <- LDA(q3_dtm,k = 4, control = list(seed = 1234))
